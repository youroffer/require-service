...
func (h *Handler) addFilter(c *gin.Context) {
	...
	filterResp, err := h.srv.AddFilter(c.Request.Context(), query.Filter)
	if err != nil {
		if errors.Is(err, repository.ErrFilterExist) {
			c.AbortWithStatusJSON(http.StatusBadRequest, errorResponse{err.Error()})
			return
		}
...
func (h *Handler) deleteFilter(c *gin.Context) {
	...
	err := h.srv.DeleteFilter(c.Request.Context(), uri.Filter)
	if err != nil {
...
func (h *Handler) getFilters(c *gin.Context) {
	...
	filterResp, err := h.srv.GetFilters(c.Request.Context(), query.Limit, query.Offset)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, errorResponse{err.Error()})
		return
	}
