...
type (
	Analytic interface {
		GetAnalytics(ctx context.Context) ([]*models.Analytic, error)
	}
...
func NewParser(cfg config.API_HH, repo Repository, cache Cache, log *logrus.Logger) *Parser {
	return &Parser{
		cfg:   cfg,
		repo:  repo,
		cache: cache,
...
func (p *Parser) Parse(ctx context.Context) {
	...
	// получаем анилитику
	analytics, err := p.repo.GetAnalytics(ctx)
	if err != nil {
		p.log.Error(err)
		return
	}
	...
	for _, analytic := range analytics {
		...
		default:
			...
			go func() {
				...
				// отправляем в кэш
				if err := p.cache.SetAnalyticWithWords(context.Background(), analyticWords, analytic.PostID); err != nil {
					logEntry.Error(err)
					return
				}
...
func (p *Parser) mustFilter() {
	...
	filters, err := p.repo.GetFilters(context.Background())
	if err != nil {
		p.log.Error(err)
	}
...
func (p *Parser) mustTopSkills(vacancies []*Vacancy, limit int) []*models.TopWords {
...
func (p *Parser) mustTopKeywords(vacancies []*Vacancy, limit int) []*models.TopWords {
...
func getTopWords(words map[string]int, limit int) []*models.TopWords {
	top := []*models.TopWords{}

	for word, reference := range words {
		top = append(top, &models.TopWords{Word: word, Reference: reference})
	}
